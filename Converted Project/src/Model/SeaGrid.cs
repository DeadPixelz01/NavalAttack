    ...
    public AttackResult HitTile(int row, int col)
    {
        try
        {
            ...
            if (_GameTiles[row, col].Shot)
                return new AttackResult(ResultOfAttack.ShotAlready,
                    "have already attacked [" + col + "," + row + "]!", row,
                    col);
            ...
            if (_GameTiles[row, col].Ship.IsDestroyed)
            {
                ...
                _ShipsKilled += 1;
                return new AttackResult(ResultOfAttack.Destroyed, _GameTiles[row, col].Ship, "destroyed the enemy's",
                    row, col);
            }
